import type { Route } from "./+types/api.update-verse-character";
import { getDb, verseCharacters } from "~/lib/db.server";
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  try {
    ...
    const { verseId, characterId, name, description } = body;
    // defaultInput can be string (URL) or null (reset to poster)
    const hasDefaultInput = "defaultInput" in body;

    if (!verseId || !characterId) {
      return Response.json({ error: "verseId and characterId are required" }, { status: 400 });
    }
    ...
    console.error("Failed to update verse character:", error);
    return Response.json(
