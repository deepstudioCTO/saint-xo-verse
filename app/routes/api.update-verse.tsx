import type { Route } from "./+types/api.update-verse";
import { getDb, verses } from "~/lib/db.server";
import { eq } from "drizzle-orm";

export async function action({ request, context }: Route.ActionArgs) {
  if (request.method !== "POST") {
    return Response.json({ error: "Method not allowed" }, { status: 405 });
  }

  try {
    const body = await request.json();
    const { verseId, name, description } = body;

    if (!verseId) {
      return Response.json({ error: "verseId is required" }, { status: 400 });
    }

    const hasDescription = "description" in body;
    if (!name && !hasDescription) {
      return Response.json(
        { error: "name or description is required" },
        { status: 400 }
      );
    }

    const db = getDb(context.cloudflare as { env: Record<string, string> });

    const updateData: { name?: string; displayName?: string; description?: string | null } = {};
    if (name && typeof name === "string" && name.trim().length > 0) {
      updateData.name = name.trim().toLowerCase();
      updateData.displayName = name.trim();
    }
    if (hasDescription) {
      updateData.description = body.description === null ? null : String(body.description).trim();
    }

    if (Object.keys(updateData).length === 0) {
      return Response.json(
        { error: "No valid fields to update" },
        { status: 400 }
      );
    }

    await db
      .update(verses)
      .set(updateData)
      .where(eq(verses.id, verseId));

    return Response.json({ success: true });
  } catch (error) {
    console.error("Failed to update verse:", error);
    return Response.json(
      { error: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
