...
async function syncDb() {
  ...
  // 1. Verses
  const dbVerses = await db.select().from(verses).orderBy(asc(verses.displayOrder));
  writeFile("synced-verses.ts", `// Auto-generated by scripts/sync-public.ts — DO NOT EDIT MANUALLY
import type { Verse } from "./types";

export const SYNCED_VERSES: Verse[] = ${JSON.stringify(
    dbVerses.map((v) => ({
      id: v.id,
      name: v.name,
...
`);
  console.log(`  ✅ synced-verses.ts (${dbVerses.length} verses)`);

  // 2. Verse Characters — poster URL → local path
  const dbVc = await db.select().from(verseCharacters).orderBy(asc(verseCharacters.displayOrder));
  writeFile("synced-verse-characters.ts", `// Auto-generated by scripts/sync-public.ts — DO NOT EDIT MANUALLY
import type { VerseCharacter } from "./types";

export const SYNCED_VERSE_CHARACTERS: VerseCharacter[] = ${JSON.stringify(
    dbVc.map((vc) => ({
      id: vc.id,
      verseId: vc.verseId,
      ...
      video: vc.video, // already local path like /character/00_sumin.mp4
      poster: supabaseUrlToLocal(vc.poster),
      defaultInput: vc.defaultInput ? supabaseUrlToLocal(vc.defaultInput) : null,
...
`);
  console.log(`  ✅ synced-verse-characters.ts (${dbVc.length} verse characters)`);
