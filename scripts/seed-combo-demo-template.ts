...
async function seed() {
  ...
  /**
   * 썸네일이 없으면 W패널이 빈 회색 카드가 되고(이름 라벨이 흰색이라) 이름도 안 보인다.
   * 초기값은 멤버 포스터지만, 포스터는 1:2라 3:4 카드에서 62%가 잘려 머리·다리가 사라진다.
   * → 실행 결과물로 교체하는 편이 낫다(2:3이라 잘림 11%, 다른 카드와 톤도 맞음).
   *   그래서 **이미 썸네일이 설정돼 있으면 덮어쓰지 않는다.**
   */
  const fallbackThumbnailUrl = persona.defaultInput ?? persona.poster;
  ...
  if (existing) {
    await db
      ...
      .set({
        ...
        edges,
        description,
        thumbnailUrl: existing.thumbnailUrl ?? fallbackThumbnailUrl,
        category: "video",
        isPublished: true,
      })
