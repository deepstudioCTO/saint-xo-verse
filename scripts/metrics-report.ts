 ...
 *
 * workflow_runs · node_runs · generations · workflow_templates를 읽어
...
import { workflowRuns, nodeRuns, generations, workflowTemplates } from "../drizzle/schema";
import {
  summarizeDurations,
  reductionRate,
...
async function main() {
  const [runs, nodes, gens, templates] = await Promise.all([
  ...
  const nodeRows = nodes.map((n) => ({
    ...
    status: n.status,
    outputs: n.outputs,
    legacyGenerationId: n.legacyGenerationId,
  }));
  const genRows = gens.map((g) => ({ type: g.type, status: g.status, createdAt: g.createdAt }));
  ...
  const report: MetricsReport = {
    ...
    outputs: summarizeOutputs(genRows, nodeRows),
    monthly: monthlyBreakdown(runRows, genRows),
    templateCounts: {
