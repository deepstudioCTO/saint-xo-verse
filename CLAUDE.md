...
| 지표 집계 | 순수 로직 `workflow/metrics.ts`(vitest) + `scripts/metrics-report.ts`(DB 읽기·`reports/metrics-<날짜>.{md,json}` 출력). **셀 수 없는 것은 세지 않는다** — 제외 행 수와 사유를 항상 함께 반환(타임스탬프 오염 6건, templateId 커버리지). 채택률은 기록 컬럼이 없어 항상 null, 단축률은 `MANUAL_BASELINE_MIN` 없으면 미산출 | 보고서 실측값의 단일 소스라 추정치가 섞이면 안 됨. 숏폼 편수는 upscale(파생물)·legacyGenerationId(중복) 제외 |
...
│   ├── workflow/        # 실행엔진 순수 로직 (서버 Workflow·클라 공유, vitest): resolveUpstreamInputs, topoSort, deriveRunStatus, spec(nodeToImageSpec), imageModels(레지스트리), presets(pickPresetParams/parsePresetBody), types
